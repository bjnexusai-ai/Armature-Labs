    ...
    >
      <div className="modal-box w-full max-w-[520px] bg-card-bg rounded-[18px] p-[26px_26px_22px] shadow-[0_30px_70px_rgba(10,30,30,0.28)] max-h-[86vh] overflow-y-auto">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h3 className="font-display text-title-sm font-bold text-ink m-0">Receive against {purchaseOrder.po_number}</h3>
        ...
        {drafts.length === 0 ? (
          <p className="text-body-sm text-ink-soft">Nothing left to receive on this purchase order.</p>
        ) : (
          <form onSubmit={handleSubmit}>
            <div className="space-y-3 mb-4">
              {drafts.map((d) => (
                <div key={d.purchaseOrderItemId} className="border border-border rounded-xl p-3">
                  <p className="text-body-sm font-semibold text-ink mb-2">
