...
export function ManufacturerDetailPage() {
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-5">
        ...
        {!editing && (
          <button
            ...
            className="px-4 py-2.5 rounded-[10px] border border-border text-[13px] font-semibold hover:bg-page-bg-top"
          >
            Edit
          </button>
        )}
      ...
      <div className="surface-card rounded-[18px] p-5 mb-5">
        {editing ? (
          ...
          <div className="grid grid-cols-2 gap-4 text-[13px]">
            <div>
              <div className="text-[11px] uppercase tracking-wider text-ink-soft mb-1">Contact</div>
              <div className="text-ink">{manufacturer.contact_name || '—'}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-wider text-ink-soft mb-1">Email</div>
              <div className="text-ink">{manufacturer.email || '—'}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-wider text-ink-soft mb-1">Phone</div>
              <div className="text-ink">{manufacturer.phone || '—'}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-wider text-ink-soft mb-1">Added</div>
      ...
      <div className="surface-card rounded-[18px] p-5 mb-5">
        <h3 className="font-display text-[15px] font-bold text-ink mb-3">Stripe Connect</h3>
          ...
          <>
            ...
            {manufacturer.connect_status === 'Onboarding' && (
              <button
                ...
                className="px-4 py-2.5 rounded-[10px] border border-border text-[13px] font-semibold hover:bg-page-bg-top disabled:opacity-60"
              >
      ...
      <div className="surface-card rounded-[18px] p-5 mb-5">
        <h3 className="font-display text-[15px] font-bold text-ink mb-3">Send a payout</h3>
        {!manufacturer.stripe_connected_account_id && (
        ...
        {payoutError && (
          <div className="text-[12.5px] text-[#9C4326] bg-[#FBEEEA] border border-[#EED0C4] rounded-xl px-3.5 py-2.5 mb-3">
            {payoutError}
          </div>
        )}
        <form onSubmit={handleCreatePayout} className="flex flex-wrap items-end gap-3">
          <div>
            <label className="block text-[12.5px] font-semibold text-ink mb-1.5">Case ID (optional)</label>
            <input
          ...
          <div>
            <label className="block text-[12.5px] font-semibold text-ink mb-1.5">Amount (USD)</label>
            <input
      ...
      <div className="surface-card rounded-[18px] p-5">
        <h3 className="font-display text-[15px] font-bold text-ink mb-3">Payout history</h3>
        {payouts.length === 0 ? (
            ...
            <tbody>
              {payouts.map((p) => (
                <tr key={p.id}>
                  ...
                  <td className="p-3 border-b border-border text-[13px] font-mono text-ink-soft">
